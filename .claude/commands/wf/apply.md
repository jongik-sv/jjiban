...
- 1단계 문서 로드에 분할 문서 3개 추가
- 4단계 설계 문서 업데이트에 분할 문서별 수정 대상 명시
- 출력 예시에 분할 문서 수정 결과 추가
Changes (v3.0):
- WP/ACT 계층 입력 지원 추가
...
Changes (v2.1):
- 적용 완료 시 파일명 변경 로직 추가: "(적용완료)" 접미사
- 리뷰 문서 탐색 시 "(적용완료)" 파일 제외 로직
- 중복 적용 방지를 위한 에러 케이스 추가
Changes (v2.0):
- 맥락 기반 선택적 적용 원칙 추가 (무조건 적용 금지)
- 6단계 구조화된 실행 플로우
- 적용/조정/보류 판단 체계
- 리뷰 문서에 적용 결과 섹션 추가 로직
- 맥락 기반 판단 체크리스트
- 우선순위별 처리 기준 상세화
- 성공 기준 및 금지 사항 명시
Reference: .claude/commands_참고/design/apply_detail_review.md
-->
