...
**WBS Task 설명에서 다음 키워드 감지**:

| 감지 대상 | 키워드 예시 |
|----------|------------|
| **화면/UI 명시** | `화면`, `UI`, `Frontend`, `프론트엔드`, `Form`, `폼` |
| **컴포넌트 언급** | `컴포넌트`, `Component`, `페이지`, `Page`, `뷰`, `View` |
| **사용자 인터랙션** | `목록`, `리스트`, `상세`, `편집`, `생성`, `삭제 버튼` |
| **레이아웃 관련** | `대시보드`, `Dashboard`, `모달`, `Modal`, `다이얼로그` |

**감지 로직**:
```
Frontend 포함 여부 = Task 설명에 위 키워드 중 하나 이상 포함
```

#### 7.2 자동 실행 흐름

```
[development Task + Frontend 감지됨]
    ↓
기본설계 문서 생성 (010-basic-design.md)
    ↓
...
🎨 Frontend 감지: 화면설계 자동 실행
├── 감지된 키워드: "Form", "목록 화면", "Frontend"
└── /wf:ui TSK-01-01-01 실행 중...
...
ℹ️ Frontend 미감지: 화면설계 생략
   (Backend only Task로 판단)

다음 단계: /wf:draft TSK-01-02-01
```

#### 7.5 수동 실행 옵션

Frontend가 감지되지 않았지만 화면설계가 필요한 경우:
```bash
/wf:ui TSK-01-02-01  # 수동으로 화면설계 실행
```
...
ℹ️ Frontend 미감지: 화면설계 생략 (Backend only Task)
