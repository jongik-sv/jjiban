...
test.describe('WBS Page Integration', () => {
  test.beforeEach(async ({ page }) => {
    ...
    const wbsContent = `# WBS - JJIBAN Project Manager
...
`
  ...
  test('TC-016: 페이지 초기화 → 프로젝트 → WBS → Task 선택 (Happy Path)', async ({ page }) => {
    // 페이지 접속
    await page.goto(`/wbs?projectId=${TEST_PROJECT_ID}`)
    ...
    await expect(header).toContainText('JJIBAN Project Manager')
  })
  ...
  test('TC-002: projectId 잘못된 형식 → Empty State', async ({ page }) => {
    ...
    await page.goto('/wbs?projectId=Invalid-Project')
